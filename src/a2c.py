...
class A2CNetSeparated(nn.Module):
    def __init__(self, nr_input_features, nr_actions):
        super(A2CNetSeparated, self).__init__()
        nr_hidden_units = 64
    ...
    def forward(self, state):
        state = state.view(state.size(0), self.nr_input_features)

        policy_dist = self.actor(state)
        value = self.critic(state)
        return F.softmax(policy_dist, dim=-1), value
...
class A2CLearner:
    def __init__(self, params):
        ...
        self.a2c_net = A2CNetSeparated(self.nr_input_features, self.nr_actions).to(
            self.device
        )
    ...
    def save_state_dict(self, model_name=None):
        torch.save(self.a2c_net, f"{self.name}_dict.pt") if not (
            model_name
        ) else torch.save(self.a2c_net, f"{model_name}_dict.pt")
    ...
    def save_net(self, model_name=None):
        torch.save(self.a2c_net, f"{self.name}_model.pt") if not (
            model_name
        ) else torch.save(self.a2c_net, f"{model_name}_model.pt")
